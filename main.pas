...
FUNCTION TfMain.CargaDataFact(opJsonFull: ISuperObject; var opTmpDb: TMemTableEh): integer;
VAR
  cField: string;
...
FUNCTION TfMain.CargaDataDet(opJsonFull: ISuperObject; var opTmpDb: TMemTableEh; cOp_id: string): integer;
VAR
  cField: string;
  cValue: string;
  vValue: variant;
...
procedure TfMain.oBtn_manualClick(Sender: TObject);
begin
  if (self.oImg_Semaf.Stretch = false) then
  begin
    self.oImg_Semaf.Picture.LoadFromFile(self.cRes_Path + '\rojo.png');
    self.oImg_Semaf.Stretch := True;
    self.oImg_Semaf.Repaint;

    self.oBtn_manual.Caption := 'Modo Automático';
  ...
  begin
    ...
    self.oImg_Semaf.Stretch := false;
    self.oImg_Semaf.Repaint;
    self.oBtn_manual.Caption := 'Modo Manual';
...
procedure TfMain.oBtn_MinimizeClick(Sender: TObject);
begin
...
procedure TfMain.oBtn_PrintClick(Sender: TObject);
begin
  self.iShoiceAct := 1;
  self.oTm_Wait_For_End_Trans.Enabled := True;
end;
...
procedure TfMain.oTm_Wait_For_End_TransTimer(Sender: TObject);
begin
...
function TfMain.BestFit(const AInput: AnsiString): AnsiString;
const
...
procedure TfMain.Hacer_Cierres;
begin
